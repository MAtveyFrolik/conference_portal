...
@login_required
def admin_dahboard_view(request):
    if not request.user.is_admin and request.user.username != 'Conf2027':
    ...
    if request.method == 'POST':
        ...
        booking.save()
        messages.success(request, f'Статус заявки обновлён на "{booking.get_status_diplay()}"')
        return redirect('bookings:admin_dahboard')
